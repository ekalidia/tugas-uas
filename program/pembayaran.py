...
def uts():
    ...
    for i in range (no):
        ...
        table.set_cols_dtype(['i','t','t','t','t','t','t','t','t','t','t'])
        table.set_cols_align(['i','c','c','c','c','c','c','c','c','c','c'])
        table.set_cols_width([10,10,10,10,10,10,10,10,10,10,10])
        table.add_rows([['No','NAMA',' NIM ','KELAS','BIAYA UTS','BIAYA SPP','SEMINAR','KAS','ADMIN','TOTAL','TANGGAL TRANSAKSI'],
                        [i+1, nama[i],nim[i],kelas[i],bayar_uts,bayar_spp,seminar_bayar,kas_bayar,admin,akhir,x]])
    print (table.draw())
    tanya()
...
def uas():
    ...
    for i in range (no):
        ...
        table.set_cols_align(['i','c','c','c','c','c','c','c','c','c','c'])
        table.set_cols_width([10,10,10,10,10,10,10,10,10,10,10])
        table.add_rows([['No','NAMA','NIM','KELAS','BIAYA UAS','BIAYA SPP','SEMINAR','KAS','ADMIN','TOTAL','TANGGAL TRANSAKSI'],
                        [i+1, nama[i],nim[i],kelas[i],bayar_uas,bayar_spp,bayar_seminar,kas_bayar,admin,akhir,x]])
    print(table.draw())
    tanya()
